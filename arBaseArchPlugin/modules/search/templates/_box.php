<div id="search-form-wrapper" role="search">
  ...
  <form action="<?php echo url_for(array('module' => 'informationobject', 'action' => 'browse')) ?>"
    data-autocomplete="<?php echo url_for(array('module' => 'search', 'action' => 'autocomplete')) ?>"
    autocomplete="off">

    <input type="hidden" name="topLod" value="0" />

    <div class="grupo-campo-busca">
      <?php if (isset($repository)): ?>
        <input type="text" name="query" <?php if (isset($sf_request->query))
          echo ' class="focused"' ?>
            value="<?php echo $sf_request->query ?>"
          placeholder="<?php echo __('Search %1%', array('%1%' => render_title($repository))) ?>" />
      <?php else: ?>
        <input type="text" name="query" <?php if (isset($sf_request->query))
          echo ' class="focused"' ?>
      ...
      <!--CUSTOMIZAÇÃO COC-->
      <!--Incluir botao de busca na interface -->
      <button type="submit" class="busca-submit" title="<?php echo __('Search') ?>"></button>
      <!--FIM CUSTOMIZAÇÃO COC-->
    </div>
    ...
    <div id="search-realm" class="search-popover">
          ...
          <div>
            <label>
              <input name="repos" checked="checked" type="radio" value="<?php echo $repository->id ?>"
                data-placeholder="<?php echo __('Search %1%', array('%1%' => render_title($repository))) ?>" />
          ...
          <div>
            <label>
              <input name="repos" type="radio" value="<?php echo $altRepository->id ?>"
                data-placeholder="<?php echo __('Search %1%', array('%1%' => render_title($altRepository))) ?>" />
      ...
      <div class="search-realm-advanced">
        <a
          href="<?php echo url_for(array('module' => 'informationobject', 'action' => 'browse', 'showAdvanced' => true, 'topLod' => false)) ?>">
